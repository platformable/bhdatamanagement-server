...
const sendMessageToOEFYipSubscriber = async (eventName,eventDate,workArea,eventDescription,locationAddress,onlineInPersonEventType,eventStartTime,eventFinishTime)=>{
  let mailTrasporter = nodemailer.createTransport({
    service:'gmail',
    auth:{
      user:process.env.NODEMAILEREMAIL,
...
END:VCALENDAR`
  ...
  let details = {
    ...
    //to: clientHCWEmail,
    to:['alexei@platformable.com'],
    subject:"A new event has been registered for the YIP program",
  icalEvent: {
    filename: `${eventName}.ics`,
    method: 'PUBLISH',
    content: calendarData,
    ...
    text:`
    Hi, a new event has been registered for the YIP program.
...
const submissionStatus='Submitted'
  ...
  updateOefYipEvent: async (req, res) => {
    let {
      createdbyLastName,
      createdByName,
      eventDate,
    ...
    try {
      ...
      eventFinishTime=$6,
      eventStartTime=$7,
      inPersonEventTypeId=$8,
      inPersonEventTypeName=$9,
      locationAddress=$10,
